...
-   Part II ([Chapter 7](at-scale.qmd) --- [Chapter 9](pipeline-args.qmd)) describes how leverage `sceptre` in conjunction with the companion package [`ondisc`](https://timothy-barry.github.io/ondisc/) to analyze data at-scale on a cluster or cloud.

-   Part III ([Chapter 10](methods.qmd) --- ) explicates the methods and algorithms that underlie `sceptre`.

Finally, the [Appendix](appendix.qmd) contains several sections, including a glossary and FAQs.

# Acknowledgments {.unnumbered}

We gratefully acknowledge Kathryn Roeder, Xihong Lin, John Morris, Kaishu Mason, Xuran Wang, and Ziang Niu for contributing to the research and development underlying `sceptre`. We additionally thank the many `sceptre` users who have provided and continue to provide valuable feedback on the package.
