...
::: callout-tip
Part I of this book describes how to apply `sceptre` to analyze single-cell CRISPR screen data on a laptop or desktop. This part of the book assumes knowledge of R at a beginner level.
:::
...
In single-cell CRISPR screen experiments viruses typically are used to deliver gRNAs to cells. In low-MOI screens the experimenter aims to deliver one virus to each cell; in high-MOI screens, by contrast, the experimenter aims to deliver *multiple* viruses (each harboring a different gRNA) to each cell. Some recent single-cell CRISPR screens involve a special design in which each virus contains a vector harboring multiple gRNAs. For example, @replogle2022 conducted a screen in which each viral vector contained two gRNAs, with each targeting the same site. In such "multi-guide vector" screens, it is important to account for the vector on which each gRNA is located to ensure that the cellwise QC and association testing steps are carried out correctly. Users can pass vector information to `sceptre` by including a `vector_id` column in the `grna_target_data_frame`, which specifies the vector on which each gRNA is located (@fig-multi_guide_vector_table).
...
`ondisc` is a companion R package to `sceptre` that enables users to analyze large-scale single-cell CRISPR screen data out-of-core on a laptop or distributed across multiple nodes on a computing cluster. `ondisc` enables the construction of a so-called "`ondisc`-backed `sceptre_object`," which is a special kind of `sceptre_object` in which the response and gRNA expression matrices are stored on-disk rather than in memory. We defer a detailed discussion of the `ondisc` package and large-scale single-cell CRISPR screen data analysis to Part II of this book (i.e., @sec-at-scale — @sec-pipeline_args). For the time being, we describe how to create an `ondisc`-backed `sceptre_object`. Users can create an `ondisc`-backed `sceptre_object` by calling a standard data import function (i.e., `import_data()` or `import_data_from_cellranger()`), setting the argument `use_ondisc` to `TRUE` and `directory_to_write` to the directory in which to write the disk-based gene and gRNA expression matrices. The table below summarizes which data import function one should call (and which arguments one should supply to that data import function) on the basis of the input data type and whether one wishes to create a standard `sceptre_object` or an `ondisc`-backed `sceptre_object`. (Note that initializing an `ondisc`-backed `sceptre_object` from the output of the Parse program is not yet implemented.)
