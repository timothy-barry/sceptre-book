...
-   `grna_matrix` is the matrix of gRNA UMI counts. The gRNAs should be in the rows and the cells in the columns. The row names of `grna_matrix` should be the gRNA IDs. The first ten rows and columns of the example `grna_matrix` are as follows.
    ...
    `response_matrix` and `grna_matrix` should be stored in one of the following formats: `matrix`, `dgTMatrix`, `dgCMatrix`, or `dgRMatrix`. **Note:** The columns of `response_matrix` and `grna_matrix` are assumed to correspond to the same ordering of cells. Please make sure to preserve the consistency of column ordering. We recommend users add cell barcodes as column names for `response_matrix` and `grna_matrix`, in which case `sceptre` will check the consistency of cell ordering on their behalf.
