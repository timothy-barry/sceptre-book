...
The outputs of the pipeline are written to `output_directory`, which, in the above script, is set to `~/sceptre_outputs`. We used the *trans* interface to the Nextflow pipeline to conduct a *trans* analysis of one of the datasets published in @replogle2022. In particular, we analyzed the so-called "rd7" dataset, which contains 616,184 cells, 36,601 genes, and 5,086 targeting gRNAs spread across 2,384 genomic targets. (The code used to analyze these data is not shown in this book.) Upon completing the analysis, the Nextflow pipeline writes the following files to the `~/sceptre_outputs` directory.
...
files <- c("analysis_summary.txt", "plot_covariates.png", "grna_assignment_matrix.rds",
           ...
           "plot_cellwise_qc.png")
print(files)
```

The files contained within this directory are similar to those outputted by the standard interface. In particular, `analysis_summary.txt` is a text file summarizing the analysis, `grna_assignment_matrix.rds` is a sparse, logical matrix containing the gRNA-to-cell assignments, etc. However, the *trans* interface to the pipeline does not (currently) render plots corresponding to the calibration check, power check, or discovery analysis. (Indeed, the *trans* interface does not carry out a calibration check or dedicated positive control analysis, and the discovery analysis might be so large that the corresponding plot is challenging to render.) 

The results of the discovery analysis are stored within the subdirectory `trans_results`. The results data frame is distributed across multiple `parquet` files, which is the file type used by the `arrow` package to store an R data frame. We print a few of the `parquet` files below.
...
```{r,eval=FALSE}
result_for_given_target <- ds |>
  filter(grna_target == "ENSG00000094914") |> 
  select(p_value, log_2_fold_change) |>
  arrange(p_value) |>
  collect()
```

Another common operation is to obtain the p-value and log fold change for all pairs containing a given *response*. We can do that as follows.

```{r,eval=FALSE}
result_for_given_response <- ds |>
...
The final verb in the chain of verbs always should be `dplyr::collect()`, which tells `arrow` to load the data frame into memory. Finally, we can load an individual `parqet` file into memory as a standard R data frame using the `read_parquet()` function.
...
::: callout-note
The *trans* interface to the Nextflow pipeline does not perform a multiplicity correction on the p-values. In particular, the p-values in the results data frame are unadjusted, and the results data frame does not contain a column `significant` indicating whether a given pair has been called as significant.
:::
