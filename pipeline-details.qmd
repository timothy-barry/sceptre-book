...
nextflow run timothy-barry/sceptre-pipeline -r main \
 ...
 --response_n_umis_range_upper 0.95 \
...
nextflow run timothy-barry/sceptre-pipeline -r main \
 ...
 --response_n_umis_range_upper 0.8 \
...
Thus, we relax the cellwise QC thresholds slightly, setting `--response_n_umis_range_lower` and `--response_n_umis_range_upper` to`0.1` and `0.9`, respectively. Additionally, we pass the `-resume` flag to the pipeline to avoid recomputing the gRNA-to-cell assignments.
...
nextflow run timothy-barry/sceptre-pipeline -r main \
 ...
 --response_n_umis_range_upper 0.9 \
 --pipeline_stop run_qc \
 -resume
```
...
```{r,echo=FALSE}
read.csv("rd7_result_for_given_target.csv")
```
...
```{r,echo=FALSE}
read.csv("rd7_result_for_given_response.csv")
```
