...
`sceptre` employs a simple and fast strategy for estimating log-fold change. Recall that, before computing the test statistics $z, z_1, \dots, z_B$, we fit the negative binomial GLM (@eq-nb_glm), yielding parameter estimates $\hat{\tau}$ and $\hat{\theta}$ for $\tau$ and $\theta$, respectively. To estimate the log-fold change, we posit the simplified model $$Y_j \sim \textrm{NB}_{\theta}(\mu_j); \quad \log(\mu_j) = \xi X_j + o_j,$$ {#eq-simplified_nb_model} where $o_j = \hat{\tau}^T Z_j$ is the $j$th fitted value of the negative binomial GLM (@eq-nb_glm). (This simplification is analogous to the simplification employed in the context of fitting the gRNA mixture model; @sec-mixture_method.) Estimating the log fold change reduces to estimating $\xi$. We treat the terms $o_1, \dots, o_n$ as i.i.d. draws from some distribution $P$; this approximation is reasoanble for large $n$.

Let $f_\theta(y_j; \mu_j)$ denote the probability mass function of the negative binomial distribution with mean $\mu_j$ and size parameter $\theta$ evaluated at $y_j$:

$$ f_\theta(y_j; \mu_j) = \binom{y_j + \theta - 1}{y_j} \left(\frac{\mu_j}{\mu_j+\theta} \right)^{y_j} \left(\frac{\theta}{\mu_j + \theta}\right)^\theta.$$ The log-likelihood of the model (@eq-simplified_nb_model) is
