...
There are several statistical tasks involved in analyzing single-cell CRISPR screen data, including assigning gRNAs to cells, testing for association between gRNAs and the expression of responses (such as genes), and integrating information across gRNAs that target the same site (and that potentially exhibit variable effectiveness). This chapter provides an overview of the statistical methods that `sceptre` employs to carry out these analysis tasks. The methods described in this and subsequent chapters were introduced and stress-tested on real data across three papers: @barry2021, @barry2022c, and @barry2023.
...
We explicate the differential expression testing and estimation paradigm of `sceptre`. First, we introduce some notation. Consider a given target-response pair. Suppose that there are $n$ cells across the treatment and control groups for this pair.

## Integrating information across (variably effective) gRNAs
