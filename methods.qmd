...
Thus, we can approximate the MLE $\hat{\xi}$ with @eq-approximate_mle:
$$
\hat{\xi} \approx \log\left(\frac{\sum_{j=1}^s Y_j }{\sum_{j=1}^s \exp[o_j] }\right).
$$

`sceptre` estimates the log fold change using this simple expression. An advantage to this estimator is that it is quite fast to compute using the fitted GLM (@eq-nb_glm).
