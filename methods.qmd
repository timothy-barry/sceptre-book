...
We next explicate the differential expression testing paradigm of `sceptre`, which forms the backbone of inference within the `sceptre` framework. `sceptre` is based on the combination of a negative binomial model and a resampling framework for differential expression, which enhances its robustness relative to a standard negative binomial model. We begin by refining the notation introduced above. Consider a given target-response pair. Suppose that there are $n$ cells across the treatment and control groups. (We are abusing notation slightly here; in a low-MOI analysis in which we are using the NT cells as our control group, the number of cells in a given pair is not equal to the number of cells in the whole dataset.) Let $Y = [Y_1, \dots, Y_n]^T$ be the vector of raw gene (or protein, etc.) UMI counts. Next, let $X = [X_1, \dots, X_n]^T$ be a binary vector indicating whether a given cell $j$ is in the treatment group ($X_j = 1$) or the control group ($X_j = 0$). Finally, let $Z_1, \dots, Z_n \in \mathbb{R}^{d}$ be the cell-specific covariates, where we have included a "1" in each $Z_j$ to serve as an intercept term. Finally, let $Z \in \mathbb{R}^{n \times d}$ denote the vectors $Z_1, \dots, Z_n$ assembled into a matrix, and let $[X, Z] \in \mathbb{R}^{n \times (d+1)}$ denote the matrix that results from concatenating $X$ and $Z$. Testing for association within `sceptre` involves three steps: first, "resampling" the treatment vector $B$ times to form "null" treatment vectors $\tilde{X}_1, \dots \tilde{X}_B$; second, evaluating a negative binomial test statistic on the observed treatment vector $X$ and null treatment vectors $\tilde{X}_1, \dots, \tilde{X}_B$; and third, comparing the observed test statistic to the null test statistics to compute a p-value. We describe each of these steps below.

### Resampling the treatment vector

The first step is to "resample" the treatment vector $X$ to form $B$ "null" treatment vectors $\tilde{X}_1, \dots, \tilde{X}_B$. The "null" treatment vectors are constructed such that they are not associated with the response vector $Y$ and thus enable a natural comparison to the observed treatment vector. `sceptre` provides two methods for resampling the observed treatment vector: permutations and conditional resampling. In the context of the `sceptre` software, one can set the `resampling_mechanism` argument of the `set_analysis_parameters()` function to `"permutations"` and `"crt"` to select between permutations and conditional resampling, respectively.

The permutation strategy simply involves permuting the $X$ vector $B$ times to construct the null treatment vectors. More precisely, among the $n!$ possible permutations of $X$, $B$ permutations are sampled randomly (with replacement and uniformly over the set of permutations) to form $\tilde{X}_1, \dots, \tilde{X}_B$. The conditional resampling strategy, meanwhile, aims to construct resamples $\tilde{X}_1, \dots, \tilde{X}_B$ *conditionally* on the cell-specific covariates $Z_1, \dots, Z_n$. To this end we model $X_j$ as a function of $Z_j$ using a logistic regression GLM:

$$
X_j \sim \textrm{Bern}(\mu_i) \quad \mu_i = \delta^TZ_j.
$$

Here, $\delta^T \in \mathbb{R}^{d}$ is an unknown regression coefficient. We produce an estimate $\hat{\delta}$ for $\delta$ by regressing $X$ onto $Z$ via a logistic GLM. Next, we evaluate the conditional mean $\hat{\mu}_j$ of $X_j$ given $Z_j$ by computing the fitted value of the logistic model:

$$
\hat{\mu}_j = \hat{\delta}^T Z_j.
$$

Finally, for each $k \in \{1, \dots, B\}$, we sample the null treatment vector $\tilde{X}_k$ from the distribution $$\left[ \textrm{Bern}(\hat{\mu}_1), \textrm{Bern}(\hat{\mu}_2), \dots, \textrm{Bern}(\hat{\mu}_n)\right]^T,$$ yielding null treatment vectors $\tilde{X}_1, \dots, \tilde{X}_B$.

### Computing the test statistics

The second step is to evaluate a negative binomial test statistic for the observed treatment vector $X$ and resampled treatment vectors $\tilde{X}_1, \dots, \tilde{X}_B$. The test statistic that `sceptre` uses is based on the following negative binomial GLM, which models the response UMI count $Y_j$ as a function of the cell-specific covariates $Z_j$:

$$ Y_j \sim \textrm{NB}_{\theta}(\mu_j) \quad \log(\mu_j) = \beta^T Z_j.$$
Here, $\theta$ is the (unknown) negative binomial size parameter and $\beta^T \in \mathbb{R}^d$ is an (unknown) regression coefficient. We first regress $Y$ onto $Z$ via a negative binomial GLM, yielding estimates $\hat{\beta}^T$ and $\hat{\theta}$ for $\beta$ and $\theta$ under the the null hypothesis of no relationship between the treatment and response. Next, we test the observed treatment vector $X$ and resampled treatment vectors $\tilde{X}_1, \dots, \tilde{X}_B$ for inclusion in the fitted GLM by running a GLM score test on each of these vectors. For a given treatment vector $X$, the GLM score test outputs a $z$-score given by

$$
z_\textrm{score} = \frac{X^T W M (Y - \hat{\mu})}{\sqrt{X^T W X - X^T W Z (Z^T W Z)^{-1} Z^T W X}}.
$$
This expression is derived in Section \nameref{sec:score_test_derivation}. Here, $W$ and $M(Y - \hat{\mu})$ are a matrix and vector, respectively, that depend on the fitted means $\hat{\mu}$, gene expressions $Y$, and estimated size $\hat{\theta}$:
...
$$
\begin{cases}
p_\textrm{right} = \frac{1}{B+1}\left(1 + \sum_{i=1}^B \mathbb{I}(z_\textrm{orig} \geq z_i)\right)
\\
p_\textrm{left} = \frac{1}{B+1} \left(1 + \sum_{i=1}^B \mathbb{I}(z_\textrm{orig} \leq z_i)\right) \\
p_\textrm{both} = 2 \cdot \textrm{min}\left\{ p_\textrm{right}, p_\textrm{left} \right\}.
\end{cases}
$$

The standard way to compute a p-value in the context of a permutation test or conditional randomization test is to directly compare the observed test statistic $z_\textrm{obs}$ to the null test statistics $z_1, \dots, z_B$. 

$$
\begin{cases}
p_\textrm{left} = \int_{-\infty}^{T_\textrm{obs}} f_{\hat{\mu}, \hat{\sigma}, \hat{\alpha}}(x) dx \\
p_\textrm{right} = \int_{T_\textrm{obs}}^{\infty} f_{\hat{\mu}, \hat{\sigma}, \hat{\alpha}}(x) dx  \\
p_\textrm{both} = 2 \cdot \textrm{min}\left\{ p_\textrm{right}, p_\textrm{left} \right\}.
\end{cases}
$$
...
A final statistical task is to integrate information across gRNAs that target the same site. `sceptre` offers three gRNA integration strategies: 
