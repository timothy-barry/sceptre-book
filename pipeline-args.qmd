...
-   `--run_association_analysis_time_per_pair`: the amount of time requested *per pair* for the "Run calibration check," "Run power check," and "Run discovery analysis" processes; default `0.1s`. `run_association_analysis_time_per_pair` and `pair_pod_size` are multiplied to calculate the total amount of time requested for the association analysis processes.
