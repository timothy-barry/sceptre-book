...
-   `--min_grna_n_umis_threshold`: a cell is flagged as containing zero gRNAs if the total UMI count across gRNAs (i.e., `grna_n_umis`) in that cell is less than `min_grna_n_umis_threshold`. `min_grna_n_umis_threshold` should be a non-negative integer.
