# Addendum: Nextflow intallation, pipeline arguments
...
Nextflow is a powerful and increasingly popular tool for orchestrating complex bioinformatic workflows. Users need to install Nextflow on their local machine and on their cluster to work through the examples in this book. We encourage users to consult the extensive and high-quality [Nextflow documentation](https://www.nextflow.io/docs/latest/index.html) for guidance on using Nextflow. Here, we briefly describe how to download, install, and configure Nextflow. Users should carry out the following sequence of steps twice: once on their local machine and once on their cluster.
...
2.  Install Java. Nextflow requires a specific distribution of Java, which can be installed as follows.
...
-   `--pipeline_stop`: the step at which to stop the pipeline, one of `set_analysis_parameters`, `assign_grnas`, `run_qc`, `run_calibration_check`, `run_power_check`, or `run_discovery_analysis`. If not supplied, defaults to `run_discovery_analysis`, causing the pipeline to run through to completion.
...
-   `--grna_pod_size`: the number of gRNAs to assign to each pod. Setting `grna_pod_size` to a large number (e.g., `1000`) reduces the amount of parallelization, while setting `grna_pod_size` to a small number (e.g., `10`) increases the amount of parallelization. By default `grna_pod_size` is set to 200. `grna_pod_size` should be a positive integer.

-   `--pair_pod_size`: the number of target-response pairs to assign to each pod for each of the association analyses (i.e., calibration check, power check, and discovery analysis). Setting `pair_pod_size` to a large number (e.g., `50000`) reduces the amount of parallelization, while setting `pair_pod_size` to a small number (e.g., `500`) increases the amount of parallelization. By default `pair_pod_size` is set to `10000`. `pair_pod_size` should be a positive integer.
...
-   `--run_association_analysis_time_per_pair`: the amount of time requested *per pair* for the "Run calibration check," "Run power check," and "Run discovery analysis" processes; default `1s`. `run_association_analysis_time_per_pair` and `pair_pod_size` are multiplied to calculate the total amount of time requested for the association analysis processes.
...
-   `--run_qc_memory`: the amount of memory requested for the "Run QC" process; default `4GB`.
