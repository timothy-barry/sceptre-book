...
See the [frequently asked questions page](faq.qmd) for tips on installing `ondisc` such that it runs as fast as possible. We can load `ondisc` and `sceptredata` by calling `library()`.
