...
The `sceptredata` package contains a downsampled version of the Gasperini data, which is stored within the `highmoi_example_data` list. Evaluating `data(highmoi_example_data, package = "sceptredata")` in the console loads the example high-MOI CRISPRi data into the global namespace.
...
`grna_target_data_frame_highmoi` contains the columns `grna_id` (the ID of an individual gRNA), `grna_target` (the genomic target of the given gRNA), and `chr`, `start`, and `end` (the chromosome, start coordinate, and end coordinate, respectively, of the target location of the given gRNA). These genomic coordinates are defined with respect to GRCh37 (hg19). Non-targeting gRNAs are assigned a `grna_target` of `"non-targeting"`.
