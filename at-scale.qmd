...
## Stepping through the pipeline piece by piece
...
We modify `launch_script.sh`, adding the line `--pipeline_stop "assign_grnas"` to the bottom of the script. This line tells Nextflow to stop at the "Assign gRNAs" step.
...
Next, we might want to run the subsequent step in the pipeline, namely "Run quality control." To this end, we update `launch_script.sh`, changing the line ` --pipeline_stop "assign_grnas"` to `--pipeline_stop "run_qc"`. Additionally, we add the line `-resume` to the launch script. The `-resume` flag in effect tells Nextflow to skip over all steps preceeding "Run QC" (including "Assign gRNAs"), as these steps already have been run.
...
```{r, out.width = "700px", fig.align="center", echo = FALSE, fig.cap=c("Nextflow output when the `-resume` flag is passed as an argument.")}
knitr::include_graphics("nf_output_2.png")
```

We launch the pipeline and display the terminal output above. Note that `assign_grnas` has been cached, indicating that this step of the pipeline has not been recomputed. In conclusion, `--pipeline_stop` and `-resume` enable one to step through the Nextflow pipeline in an incremental way, verifying that a given step of the pipeline looks good before proceeding to the next step. If a given step of the pipeline produces an unexpected or suboptimal result, one can update the arguments passed to that step of the pipeline, as described in the next chapter.
