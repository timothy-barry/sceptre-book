...
::: callout-note
Nextflow better understands absolute file paths than relative file paths. Thus, we recommend using `$HOME` in place of `~` in the pipeline invocation script.
:::
...
Nextflow is a powerful and increasingly popular tool for orchestrating complex bioinformatic workflows. Users will need to install Nextflow on their local machine and on their cluster to use the `sceptre` Nextflow pipeline. We encourage readers to consult the extensive and high-quality [Nextflow documentation](https://www.nextflow.io/docs/latest/index.html). Here, we briefly describe how to download, install, and configure Nextflow.
...
Configuring Nextflow on a given cluster often takes a bit of effort, but fortunately, it is a task that only needs to be completed once. For help with Nextflow configuration, we recommend reading through the Nextflow documentation, contacting a cluster administrator, pinging the (very helpful and responsive) Nextflow community (for example on [Github issues](https://github.com/nextflow-io/nextflow/issues)), or consulting with ChatGPT (seriously; it seems to know a lot about Nextflow).
