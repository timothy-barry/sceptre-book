...
Of these parameters, only `discovery_pairs` is required; the rest have sensible defaults. We describe these arguments below.

None of these parameters are required, as they all have sensible defaults. However, in most cases, users will want to specify at least `discovery_pairs`. We describe these arguments below.
...
Multiple gRNAs typically are designed to target a given genomic element. The parameter `grna_integration_strategy` controls if and how gRNAs that target the same genomic element are combined. In most cases, users wish to test whether perturbation of a target impacts the response to which it is paired. In these cases, users need to choose a strategy for integrating information across gRNAs with a given target. If users expect that gRNAs sharing a target will have similar effects on the response to which they are paired, then they should choose `grna_integration_strategy = "union"`. If users expect that gRNAs sharing a target will have substantially different effects on the response to which they are paired (e.g. due to variable gRNA effectiveness), then they should choose `grna_integration_strategy = "bonferroni"` (**Note:** The `bonferroni` option will be implemented soon). In some cases, users may wish to test each individual gRNA against the responses to which its target is paired. For example, users may wish to assess the variability in the effects of different gRNAs with the same target. In these cases, users should choose `grna_integration_strategy = "singleton"`. 

```{r, out.width = "350px", fig.align="center", echo = FALSE, fig.cap="Schematic of the choices for gRNA integration strategy."}
knitr::include_graphics("grna_integration_strategy_dec_tree.jpg")
```
...
## Control group {#sec-set_analysis_parameters_control_group}
...
## Fit parametric curve {#sec-set_analysis_parameters_parametric_curve}
